...
describe('M11 additive AI schema and authority',()=>{
 it('keeps both applied M11 migrations unchanged and adds one fifteenth repair migration',()=>{expect(migrations()).toHaveLength(15);expect(migrations().at(-3)).toBe('20260922000002_multiplayer_ai_players.sql');expect(migrations().at(-2)).toBe('20260923000000_fix_multiplayer_ai_sql_ambiguity.sql');expect(migrations().at(-1)).toBe('20260923000001_fix_start_game_coalesce.sql');expect(contentFingerprint(migration.replace(/\r\n/g,'\n'))).toBe('0dc12235');expect(contentFingerprint(repairMigration.replace(/\r\n/g,'\n'))).toBe('4337a13b');});
