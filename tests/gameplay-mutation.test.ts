...
describe('atomic gameplay mutation database contract',()=>{
 it('adds exactly one Milestone 8 migration after the unchanged Milestone 7 migration',()=>{const names=readdirSync(new URL('../supabase/migrations/',import.meta.url),{withFileTypes:true}).filter(file=>!file.isDirectory()&&file.name.endsWith('.sql')).map(file=>file.name).sort();expect(names.at(-1)).toBe('20260921000001_gameplay_mutation.sql');expect(names.at(-2)).toBe('20260921000000_active_game_state_read.sql');expect(names).toHaveLength(10);expect(readMigration).toContain('create function public.get_active_game_state_server');});
...
describe('multiplayer action UI contract',()=>{
 ...
 it('renders both resolution controls, immediate returned state, stale messaging, and no polling/Realtime/debug controls',()=>{expect(shell).toContain('data-bless-option');expect(shell).toContain('data-smite-resource');expect(shell).toContain('renderActiveGameState(code,next)');expect(clientSource).toContain('Game state changed. Refresh and try again.');const active=shell.slice(shell.indexOf('async function renderActiveGame'));expect(active).not.toMatch(/setInterval|subscribe\(|channel\(/);for(const forbidden of ['Save','Resume','Export','Import','New Game','AI delay','debug JSON'])expect(active).not.toContain(forbidden);});
