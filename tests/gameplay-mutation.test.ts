...
describe('atomic gameplay mutation database contract',()=>{
 it('retains Milestone 8 unchanged before the Realtime and AI migrations',()=>{const names=readdirSync(new URL('../supabase/migrations/',import.meta.url),{withFileTypes:true}).filter(file=>!file.isDirectory()&&file.name.endsWith('.sql')).map(file=>file.name).sort();expect(names.at(-1)).toBe('20260922000002_multiplayer_ai_players.sql');expect(names.at(-2)).toBe('20260922000001_realtime_lobby_sync.sql');expect(names.at(-3)).toBe('20260922000000_realtime_game_sync.sql');expect(names.at(-4)).toBe('20260921000001_gameplay_mutation.sql');expect(names.at(-5)).toBe('20260921000000_active_game_state_read.sql');expect(names).toHaveLength(13);expect(readMigration).toContain('create function public.get_active_game_state_server');});
