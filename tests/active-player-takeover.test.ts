...
describe('active human to AI migration contract',()=>{
 it('retains the sixteenth migration before lobby discovery',()=>{const names=readdirSync(new URL('../supabase/migrations/',import.meta.url),{withFileTypes:true}).filter(entry=>!entry.isDirectory()&&entry.name.endsWith('.sql')).map(entry=>entry.name).sort();expect(names).toHaveLength(17);expect(names.at(-2)).toBe('20260923000002_active_player_ai_takeover.sql');expect(names.at(-1)).toBe('20260924000000_lobby_discovery_and_host_moderation.sql');expect(migration).not.toMatch(/alter publication|create publication/i);expect(realtime).toContain("alter publication supabase_realtime add table public.room_game_updates");});
