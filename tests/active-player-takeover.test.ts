...
describe('active human to AI migration contract',()=>{
 it('adds exactly one sixteenth forward migration without modifying publication design',()=>{const names=readdirSync(new URL('../supabase/migrations/',import.meta.url),{withFileTypes:true}).filter(entry=>!entry.isDirectory()&&entry.name.endsWith('.sql')).map(entry=>entry.name).sort();expect(names).toHaveLength(16);expect(names.at(-1)).toBe('20260923000002_active_player_ai_takeover.sql');expect(migration).not.toMatch(/alter publication|create publication/i);expect(realtime).toContain("alter publication supabase_realtime add table public.room_game_updates");});
