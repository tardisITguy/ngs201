...
describe('Start Edge and room UI',()=>{
 ...
 it('routes active rooms to the trusted read-only game view without canonical reads',()=>{expect(shell).toContain("if(lobby.room.status==='active')");expect(shell).toContain('return renderActiveGame(lobby.room.code)');expect(shell).toContain('renderBoard(view,{}, {interactive:false})');expect(lobby).not.toContain("from('room_states')");});
});
