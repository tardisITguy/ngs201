...
describe('Start Edge and room UI',()=>{
 ...
 it('routes active rooms to the trusted game view without canonical browser reads',()=>{expect(shell).toContain("if(lobby.room.status==='active')");expect(shell).toContain('return renderActiveGame(lobby.room.code)');expect(shell).toContain('renderBoard(view,{start:startCell,end:endCell},{interactive:canPlace})');expect(lobby).not.toContain("from('room_states')");});
});
