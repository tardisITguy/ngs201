...
describe('server-side Worship Me! candidate builder',()=>{
 it('directly reuses createGame and preserves ordered identity, seed, and initial invariants',()=>{
  ...
  const state=buildWorshipMeInitialState([{userId:'a',displayName:'Alice',playerColor:'red'},{userId:'b',displayName:'Bob',playerColor:'blue'},{userId:'c',displayName:'Carol',playerColor:'purple'}],'server-seed');
...
describe('atomic Start migration',()=>{
 it('adds only the next Start migration',()=>{const names=readdirSync(new URL('../supabase/migrations/',import.meta.url),{withFileTypes:true}).filter(e=>!e.isDirectory()&&e.name.endsWith('.sql')).map(e=>e.name).sort();expect(names.at(-1)).toBe('20260920000004_start_game.sql');expect(names).toHaveLength(8);});
...
describe('Start Edge and room UI',()=>{
 ...
 it('uses stable privileged roster data, a server seed, createGame candidate, and final RPC',()=>{expect(edge).toContain(".order('joined_at').order('user_id')");expect(edge).toContain('crypto.randomUUID()');expect(edge).toContain('buildWorshipMeInitialState(players,seed)');expect(edge).toContain("serverClient.rpc('start_game_server'");expect(edge).toContain('p_expected_players:players');expect(edge).toContain('p_game_state:gameState');});
 ...
 it('renders active rooms by persisted turn order without lobby mutation controls or canonical reads',()=>{expect(shell).toContain("if(lobby.room.status==='active')");expect(shell).toContain('a.turnOrder??Number.MAX_SAFE_INTEGER');expect(shell).toContain('Canonical game state has been initialized.');expect(shell).toContain('Gameplay synchronization is coming next.');expect(lobby).not.toContain("from('room_states')");});
});
