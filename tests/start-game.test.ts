...
describe('atomic Start migration',()=>{
 it('retains exactly one original Start migration plus forward migrations',()=>{const names=readdirSync(new URL('../supabase/migrations/',import.meta.url),{withFileTypes:true}).filter(e=>!e.isDirectory()&&e.name.endsWith('.sql')).map(e=>e.name).sort();expect(names.filter(name=>name.endsWith('_start_game.sql'))).toEqual(['20260920000004_start_game.sql']);expect(names).toContain('20260921000001_gameplay_mutation.sql');expect(names).toContain('20260922000000_realtime_game_sync.sql');expect(names).toContain('20260922000001_realtime_lobby_sync.sql');expect(names.at(-7)).toBe('20260922000002_multiplayer_ai_players.sql');expect(names.at(-6)).toBe('20260923000000_fix_multiplayer_ai_sql_ambiguity.sql');expect(names.at(-5)).toBe('20260923000001_fix_start_game_coalesce.sql');expect(names.at(-4)).toBe('20260923000002_active_player_ai_takeover.sql');expect(names.at(-3)).toBe('20260924000000_lobby_discovery_and_host_moderation.sql');expect(names.at(-2)).toBe('20260924000001_return_finished_game_to_lobby.sql');expect(names.at(-1)).toBe('20260924000002_fix_return_to_lobby_signal_read.sql');expect(names).toHaveLength(19);});
