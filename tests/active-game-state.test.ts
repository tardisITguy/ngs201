...
describe('service-only active-state read contract',()=>{
 it('retains the Milestone 7 migration and all forward history',()=>{const names=readdirSync(new URL('../supabase/migrations/',import.meta.url),{withFileTypes:true}).filter(file=>!file.isDirectory()&&file.name.endsWith('.sql')).map(file=>file.name).sort();expect(names.at(-10)).toBe('20260921000000_active_game_state_read.sql');expect(names.at(-9)).toBe('20260921000001_gameplay_mutation.sql');expect(names.at(-8)).toBe('20260922000000_realtime_game_sync.sql');expect(names.at(-7)).toBe('20260922000001_realtime_lobby_sync.sql');expect(names.at(-6)).toBe('20260922000002_multiplayer_ai_players.sql');expect(names.at(-5)).toBe('20260923000000_fix_multiplayer_ai_sql_ambiguity.sql');expect(names.at(-4)).toBe('20260923000001_fix_start_game_coalesce.sql');expect(names.at(-3)).toBe('20260923000002_active_player_ai_takeover.sql');expect(names.at(-2)).toBe('20260924000000_lobby_discovery_and_host_moderation.sql');expect(names.at(-1)).toBe('20260924000001_return_finished_game_to_lobby.sql');expect(names).toHaveLength(18);});
