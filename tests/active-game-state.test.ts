...
describe('service-only active-state read contract',()=>{
 it('retains the Milestone 7 migration before Milestone 8',()=>{const names=readdirSync(new URL('../supabase/migrations/',import.meta.url),{withFileTypes:true}).filter(file=>!file.isDirectory()&&file.name.endsWith('.sql')).map(file=>file.name).sort();expect(names.at(-2)).toBe('20260921000000_active_game_state_read.sql');expect(names.at(-1)).toBe('20260921000001_gameplay_mutation.sql');expect(names).toHaveLength(10);});
...
describe('active read-only room page',()=>{
 it('fetches trusted state only after active lobby status and renders status, board, version, viewer and turn',()=>{expect(shell).toContain("if(lobby.room.status==='active')");expect(shell).toContain('return renderActiveGame(lobby.room.code)');expect(shell).toContain('STATE VERSION ${result.stateVersion}');expect(shell).toContain('Current Turn:');expect(shell).toContain('You:');expect(shell).toContain('renderBoard(view,{start:startCell,end:endCell},{interactive:canPlace})');});
 it('has manual refresh with no polling, Realtime, or lobby mutation controls',()=>{const active=shell.slice(shell.indexOf('async function renderActiveGame'));expect(active).toContain('data-refresh-game');expect(active).toContain("refresh.textContent='REFRESHING…'");expect(active).not.toMatch(/setInterval|subscribe\(|channel\(/);for(const control of ['data-leave','data-ready','data-start','data-player-color-select'])expect(active).not.toContain(control);});
