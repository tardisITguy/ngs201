import type {Cell,EdgeMarker,Modifier,Phase,PlacementAction,Player,QueuedAction} from '../engine/types';
...
export interface BoardRenderState{players:Array<Pick<Player,'id'|'name'|'color'>>;board:Cell[];phase:Phase;actionQueue:QueuedAction[];resolutionIndex:number;edgeMarkers:EdgeMarker[];tileClaims:Record<string,Modifier>}
export interface BoardRenderOptions{interactive?:boolean}
...
function markerStatus(s:BoardRenderState,index:number):MarkerStatus{return index<s.resolutionIndex?'resolved':index===s.resolutionIndex&&s.phase==='resolution'?'resolving':'pending'}
function playerLabel(s:BoardRenderState,playerId:string){const p=s.players.find(x=>x.id===playerId);return p?`${cap(p.color)} (${p.name})`:playerId}
function tokenImage(kind:'bless'|'smite'){return `<img src="${ACTION_TOKEN_ASSETS[kind]}" alt="" aria-hidden="true">`}

function tileTokens(c:Cell,s:BoardRenderState){return s.actionQueue.map((queued,index)=>({queued,index})).filter(({queued})=>queued.placement.type==='placeTile'&&queued.placement.cellId===c.id).map(({queued,index})=>{
...
function edgeAnchor(s:BoardRenderState,action:EdgeAction){const [aId,bId]=edgeEndpoints(action),a=s.board.find(c=>c.id===aId)!,b=s.board.find(c=>c.id===bId)!;return a.row===b.row?(a.col<b.col?{cell:a,side:'right' as const}:{cell:b,side:'right' as const}):(a.row<b.row?{cell:a,side:'down' as const}:{cell:b,side:'down' as const})}
function blessDirection(s:BoardRenderState,action:Extract<EdgeAction,{kind:'bless'}>){const from=s.board.find(c=>c.id===action.from)!,to=s.board.find(c=>c.id===action.to)!;if(to.col>from.col)return'right';if(to.col<from.col)return'left';if(to.row>from.row)return'down';return'up'}
function edgeTokens(c:Cell,s:BoardRenderState){return s.actionQueue.map((queued,index)=>({queued,index})).filter(({queued})=>queued.placement.type==='placeEdge'&&edgeAnchor(s,queued.placement).cell.id===c.id).map(({queued,index})=>{
...
function smiteBar(c:Cell,s:BoardRenderState){return s.edgeMarkers.filter(e=>e.kind==='smite'&&(e.a===c.id||e.b===c.id)).map(e=>{const other=s.board.find(x=>x.id===(e.a===c.id?e.b:e.a))!;if(other.row===c.row&&other.col===c.col+1)return'<span class="edge-smite edge-smite-right" aria-label="Smited edge: impassable"></span>';if(other.col===c.col&&other.row===c.row+1)return'<span class="edge-smite edge-smite-down" aria-label="Smited edge: impassable"></span>';return''}).join('')}

export function renderBoard(s:BoardRenderState,selection:{start?:string;end?:string}={},options:BoardRenderOptions={}){
 const interactive=options.interactive!==false,tag=interactive?'button':'div';
 return `<section class="board">${s.board.map(c=>{
  ...
  return `<${tag} class="cell ${c.visibleKind}${selected?' selected':''}${c.tileModifier||claim?' round-marked':''}${full?' full':''}${edges?' has-edge-token':''}" ${interactive?`data-cell="${c.id}"`:`data-coordinate="${c.id}" role="img" aria-label="${cap(c.visibleKind)} tile ${c.id}"`} style="${owner?`--temple-color:${owner.color};`:''}${marker?`--marker-color:${marker.color}`:''}"><span class="tile-coordinate" aria-label="Tile coordinate ${c.id}">${c.id}</span>${smiteBar(c,s)}${edges}${tileTokens(c,s)}${label?`<strong class="selection-label">${label}</strong>`:''}${occupancyLabel}<b>${c.visibleKind==='hidden'?'?':c.visibleKind}</b>${owner?`<small>${esc(owner.name)} · ${owner.color}</small>`:''}<span>${c.villagers.map(v=>`<i class="pawn" style="--c:${v}"></i>`).join('')}${c.priests.map(v=>`<i class="pawn priest" style="--c:${v}">P</i>`).join('')}</span><small>🌾${c.wheat} 🍞${c.bread}</small>${tileEquation(c,s)?`<small class="equation">${tileEquation(c,s)}</small>`:''}</${tag}>`;
 }).join('')}</section>`;
}
