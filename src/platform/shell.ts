...
import {createSendRoomChatMessageAction,RoomChatSendError} from './rooms/sendRoomChatMessage';
import {createRoomChatSyncCoordinator} from './rooms/roomChatSync';
import {subscribeRoomChatMessages} from './rooms/subscribeRoomChatMessages';
import {getRoomChatMessages} from './rooms/getRoomChatMessages';
import {describeBlessEdgeOption} from './rooms/blessEdgeOptionLabel';
import type {ActiveGameStateResult,RoomChatMessage,RoomChatSyncStatus,RoomGameSyncStatus,RoomLobbySyncStatus} from './types';
...
import {bindWorshipMeHelpModal,createWorshipMeHelpController,restoreWorshipMeHelpFocus,WORSHIP_ME_HOW_TO_PLAY_SECTIONS,WORSHIP_ME_TUTORIAL_STEPS,type WorshipMeHelpModalBinding,type WorshipMeHelpSection} from '../games/worship-me/ui/howToPlay';
...
import './roomChat.css';
import '../games/worship-me/ui/howToPlay.css';
...
export function startPlayShell(root:HTMLDivElement){
 const help=createWorshipMeHelpController();
 let helpModalBinding:WorshipMeHelpModalBinding|undefined,helpOpener:HTMLElement|undefined;
 ...
 const sendChat=createSendRoomChatMessageAction();
 ...
 let chatMessages:RoomChatMessage[]=[],chatStatus:RoomChatSyncStatus='unavailable',chatError='',chatSendPending=false,chatRoomCode='';
 ...
 const roomChatSync=createRoomChatSyncCoordinator({onMessages:messages=>{chatMessages=messages;renderChatPanels();},onStatus:status=>{chatStatus=status;renderChatPanels();},onError:()=>{chatError='Unable to refresh chat.';renderChatPanels();}},{fetchHistory:roomId=>getRoomChatMessages(roomId),subscribe:options=>subscribeRoomChatMessages(options)});
 ...
 const helpSection=(section:WorshipMeHelpSection)=>`<section class="worship-help__section"><h2>${escapeHtml(section.title)}</h2>${section.paragraphs.map(paragraph=>`<p>${escapeHtml(paragraph)}</p>`).join('')}${section.bullets?.length?`<ul>${section.bullets.map(item=>`<li>${escapeHtml(item)}</li>`).join('')}</ul>`:''}</section>`;
 const restoreHelpFocus=()=>{const opener=helpOpener;helpOpener=undefined;restoreWorshipMeHelpFocus(opener);};
 const renderHelpOverlay=()=>{helpModalBinding?.release();helpModalBinding=undefined;root.querySelector('[data-worship-help]')?.remove();const state=help.state;if(!state.mode){restoreHelpFocus();return;}const finalTutorialStep=state.mode==='tutorial'&&state.tutorialStep===WORSHIP_ME_TUTORIAL_STEPS.length-1;if(state.mode==='tutorial'){const step=WORSHIP_ME_TUTORIAL_STEPS[state.tutorialStep],last=finalTutorialStep;root.insertAdjacentHTML('beforeend',`<div class="worship-help-backdrop" data-worship-help><section class="worship-help worship-help--tutorial" role="dialog" aria-modal="true" aria-labelledby="worship-tutorial-heading" tabindex="-1"><header class="worship-help__header"><div><p class="eyebrow">GUIDED TUTORIAL</p><h1 id="worship-tutorial-heading">${escapeHtml(step.title)}</h1></div><span class="worship-help__progress">${state.tutorialStep+1} / ${WORSHIP_ME_TUTORIAL_STEPS.length}</span></header><div class="worship-help__body">${step.paragraphs.map(paragraph=>`<p>${escapeHtml(paragraph)}</p>`).join('')}${step.bullets?.length?`<ul>${step.bullets.map(item=>`<li>${escapeHtml(item)}</li>`).join('')}</ul>`:''}</div><footer class="worship-help__footer"><button class="text-button" type="button" data-tutorial-skip>SKIP TUTORIAL</button><div class="worship-help__nav">${state.tutorialStep?'<button class="secondary-button" type="button" data-tutorial-back>BACK</button>':''}<button class="primary-button" type="button" ${last?'data-tutorial-finish':'data-tutorial-next'}>${last?'START PLAYING':'NEXT'}</button></div></footer></section></div>`);}else{root.insertAdjacentHTML('beforeend',`<div class="worship-help-backdrop" data-worship-help><section class="worship-help worship-help--guide" role="dialog" aria-modal="true" aria-labelledby="worship-guide-heading" tabindex="-1"><header class="worship-help__header"><div><p class="eyebrow">HOW TO PLAY</p><h1 id="worship-guide-heading">WORSHIP ME!</h1></div><button class="worship-help__close" type="button" data-guide-close aria-label="Close How to Play">×</button></header><div class="worship-help__body">${WORSHIP_ME_HOW_TO_PLAY_SECTIONS.map(helpSection).join('')}</div><footer class="worship-help__footer"><button class="secondary-button" type="button" data-replay-tutorial>REPLAY TUTORIAL</button><button class="primary-button" type="button" data-guide-close>CLOSE</button></footer></section></div>`);}const overlay=root.querySelector<HTMLElement>('[data-worship-help]')!,dialog=overlay.querySelector<HTMLElement>('[role="dialog"]')!,application=root.querySelector<HTMLElement>('.play-shell')!;overlay.onclick=event=>event.stopPropagation();overlay.querySelectorAll<HTMLButtonElement>('[data-guide-close]').forEach(control=>control.onclick=()=>{help.closeGuide();renderHelpOverlay();});const replay=overlay.querySelector<HTMLButtonElement>('[data-replay-tutorial]');if(replay)replay.onclick=()=>{help.replayTutorial(state.roomCode);renderHelpOverlay();};const back=overlay.querySelector<HTMLButtonElement>('[data-tutorial-back]');if(back)back.onclick=()=>{help.back();renderHelpOverlay();};const next=overlay.querySelector<HTMLButtonElement>('[data-tutorial-next]');if(next)next.onclick=()=>{help.next();renderHelpOverlay();};const skip=overlay.querySelector<HTMLButtonElement>('[data-tutorial-skip]');if(skip)skip.onclick=()=>{help.skip();renderHelpOverlay();};const finish=overlay.querySelector<HTMLButtonElement>('[data-tutorial-finish]');if(finish)finish.onclick=()=>{help.finish();renderHelpOverlay();};helpModalBinding=bindWorshipMeHelpModal(application,overlay,dialog,state.mode==='tutorial'?(finalTutorialStep?'[data-tutorial-finish]':'[data-tutorial-next]'):'[data-guide-close]');};
 const bindHowToPlay=(roomCode?:string)=>{root.querySelectorAll<HTMLButtonElement>('[data-how-to-play]').forEach(control=>control.onclick=()=>{helpOpener=control;help.openGuide(roomCode);renderHelpOverlay();});renderHelpOverlay();};
 root.addEventListener('keydown',event=>{if(event.key!=='Escape'||!help.state.mode)return;event.preventDefault();if(help.state.mode==='tutorial')help.skip();else help.closeGuide();renderHelpOverlay();});

 const chatPanel=()=>`<section class="panel room-chat" data-room-chat><h2>ROOM CHAT</h2><div class="room-chat__body" data-chat-list></div><form class="room-chat__form" data-chat-form><label class="sr-only" for="room-chat-message">Message</label><input id="room-chat-message" data-chat-input maxlength="500" autocomplete="off" placeholder="Message..."><button class="primary-button compact" type="submit" data-chat-send>SEND</button></form><p class="form-message" data-chat-message aria-live="polite"></p><div data-chat-status></div></section>`;
  const renderChatPanels=()=>root.querySelectorAll<HTMLElement>('[data-room-chat]').forEach(panel=>{const previous=panel.querySelector<HTMLInputElement>('[data-chat-input]')?.value??'',oldList=panel.querySelector<HTMLElement>('[data-chat-list]'),nearBottom=!oldList||oldList.scrollHeight-oldList.scrollTop-oldList.clientHeight<48;panel.innerHTML=`<h2>ROOM CHAT</h2><div class="room-chat__body" data-chat-list>${chatMessages.length?chatMessages.map(message=>`<article class="room-chat__message"><strong>${escapeHtml(message.senderDisplayName)}</strong><p>${escapeHtml(message.messageText)}</p></article>`).join(''):'<p class="room-chat__empty">No messages yet.</p>'}</div><form class="room-chat__form" data-chat-form><label class="sr-only" for="room-chat-message">Message</label><input id="room-chat-message" data-chat-input maxlength="500" autocomplete="off" placeholder="Message..." value="${escapeHtml(previous)}" ${chatSendPending?'disabled':''}><button class="primary-button compact" type="submit" data-chat-send ${chatSendPending?'disabled':''}>${chatSendPending?'SENDING…':'SEND'}</button></form><p class="form-message" data-chat-message aria-live="polite">${escapeHtml(chatError)}</p><div data-chat-status>${chatStatus==='connecting'?'<small>Connecting chat…</small>':chatStatus==='unavailable'?'<small>Chat connection unavailable.</small> <button class="text-button" type="button" data-refresh-chat>REFRESH CHAT</button>':''}</div>`;const list=panel.querySelector<HTMLElement>('[data-chat-list]')!;if(nearBottom)list.scrollTop=list.scrollHeight;const form=panel.querySelector<HTMLFormElement>('[data-chat-form]')!,input=panel.querySelector<HTMLInputElement>('[data-chat-input]')!;form.onsubmit=async event=>{event.preventDefault();if(chatSendPending)return;const text=input.value;chatSendPending=true;chatError='';renderChatPanels();try{const accepted=await sendChat({roomCode:chatRoomCode,messageText:text});if(accepted){root.querySelectorAll<HTMLInputElement>('[data-chat-input]').forEach(control=>{control.value='';});chatError='';roomChatSync.acceptTrusted(accepted);}}catch(error){chatError=error instanceof RoomChatSendError?error.message:'Unable to send message.';}finally{chatSendPending=false;renderChatPanels();}};const refresh=panel.querySelector<HTMLButtonElement>('[data-refresh-chat]');if(refresh)refresh.onclick=()=>void roomChatSync.refresh();});
 const ensureChat=(lobby:Lobby)=>{chatRoomCode=lobby.room.code;if(roomChatSync.isCurrent(lobby.room.id)){renderChatPanels();return;}chatMessages=[];chatError='';chatStatus='connecting';void roomChatSync.enter(lobby.room.id,lobby.room.code);};

 async function render(route:Route){
  help.closeGuide();
  helpModalBinding?.release();helpModalBinding=undefined;helpOpener=undefined;root.querySelector('[data-worship-help]')?.remove();
  if(route.name==='room'){
   await roomChatSync.leave();chatMessages=[];chatError='';
   lobbyPresence.leave();
   gamePresence.leave();
 ...
 function renderLobbyState(lobby:Lobby,lobbyNotice=''){
   ...
   if(lobby.room.status!=='lobby'){lobbyPresence.leave();void Promise.all([roomKickSync.leave(),roomLobbySync.leave(),roomChatSync.leave()]);root.querySelector('.lobby')!.innerHTML=`<section class="status-panel"><h1>Room unavailable</h1><p>This room is no longer an active lobby.</p><a class="text-link" href="/games" data-link>Back to Games</a></section>`;return;}
   ...
   const roomName=lobby.room.roomName,roomNameHeading=roomName?`<strong class="lobby-room-name">${escapeHtml(roomName)}</strong>`:'';
   const nameRoom=lobby.room.isCurrentUserHost?`<div class="room-name-editor"><label for="room-name"><span>ROOM NAME</span><input id="room-name" data-room-name maxlength="50" autocomplete="off" value="${escapeHtml(roomName??'')}"></label><button class="secondary-button compact" type="button" data-save-room-name>SAVE NAME</button>${roomName?'<button class="text-button" type="button" data-clear-room-name>CLEAR NAME</button>':''}</div>`:'';
   ...
   const accessControl=lobby.room.isCurrentUserHost?`<label class="room-access-control"><span>ROOM ACCESS</span><select data-room-join-mode aria-label="Room access"><option value="public" ${lobby.room.joinMode==='public'?'selected':''}>PUBLIC</option><option value="code" ${lobby.room.joinMode==='code'?'selected':''}>CODE ONLY</option></select></label>`:`<span class="room-access-badge"><small>ROOM ACCESS</small>${lobby.room.joinMode==='public'?'PUBLIC':'CODE ONLY'}</span>`;
    const lobbyStatus=aggregateLobbySyncStatus();root.querySelector('.lobby')!.innerHTML=`<section class="lobby-controls" aria-labelledby="lobby-controls-heading"><div class="lobby-controls__heading"><div><h1 id="lobby-controls-heading">LOBBY CONTROLS</h1>${roomNameHeading}</div><span class="room-sync-status" data-sync-status="lobby" data-sync-state="${lobbyStatus}">${syncLabel(lobbyStatus)}</span></div><div class="lobby-controls__bar"><div class="lobby-controls__room"><span class="lobby-controls__label">ROOM CODE</span><strong class="lobby-controls__code">${escapeHtml(lobby.room.code)}</strong><button class="secondary-button compact" type="button" data-copy>Copy Code</button>${accessControl}</div><div class="lobby-controls__actions"><button class="secondary-button compact" type="button" data-how-to-play>HOW TO PLAY</button><button class="secondary-button compact" type="button" data-leave>LEAVE LOBBY</button><span class="ready-control"><button class="secondary-button compact" type="button" data-ready ${!currentPlayer||(!currentPlayer.isReady&&!canReady)?'disabled':''} ${readyHelp?'aria-describedby="ready-help"':''}>${readyLabel}</button>${readyHelp}</span>${startGame}</div></div>${nameRoom}${hostHelp}<div class="lobby-controls__messages"><p class="form-message" data-lobby-message aria-live="polite">${escapeHtml(lobbyNotice)}</p><p class="form-message" data-leave-message aria-live="polite"></p></div></section><section class="lobby-content-grid"><div class="panel players-panel"><div class="panel-title"><h2>PLAYERS</h2><div>${addAI}<button class="text-button" data-refresh>Refresh</button></div></div><ul class="player-list">${players}</ul></div>${chatPanel()}</section>`;
    renderChatPanels();
    bindHowToPlay(lobby.room.code);
   ...
   leaveButton.onclick=async()=>{
    ...
    try{const result=await leave({roomCode:lobby.room.code});if(result){await Promise.all([roomKickSync.leave(),roomChatSync.leave()]);router.navigate('/games/worship-me');}}
   ...
   const roomNameInput=root.querySelector<HTMLInputElement>('[data-room-name]'),saveRoomName=root.querySelector<HTMLButtonElement>('[data-save-room-name]'),clearRoomName=root.querySelector<HTMLButtonElement>('[data-clear-room-name]');
   const submitRoomName=async(value:string|null)=>{if(!roomNameInput||!saveRoomName)return;roomNameInput.disabled=true;saveRoomName.disabled=true;if(clearRoomName)clearRoomName.disabled=true;saveRoomName.textContent='SAVING…';lobbyMessage.textContent=value===null?'Clearing room name...':'Saving room name...';try{await manageLobby({roomCode:lobby.room.code,command:{type:'setRoomName',roomName:value}});pendingLobbyNotice='';await roomLobbySync.refresh();}catch(error){pendingLobbyNotice=error instanceof ManageLobbyError?error.message:'Unable to update room name.';await roomLobbySync.refresh();}};
   if(saveRoomName&&roomNameInput)saveRoomName.onclick=()=>void submitRoomName(roomNameInput.value);
   if(clearRoomName)clearRoomName.onclick=()=>void submitRoomName(null);
