...
export function renderLobbyPlayerRows(players:LobbyPlayer[],isHost=false):string{
 ...
 return players.map(player=>{
  ...
  if(player.control==='human')return`<li data-player-row="${escapeHtml(player.userId)}" ${player.isCurrentUser?'data-current-player':''}><strong class="player-name">${escapeHtml(player.displayName)}</strong><span class="player-row-meta">${player.isHost?'<span class="host-badge">HOST</span>':''}${player.isCurrentUser?colorControl(player.playerColor,unavailable,'data-player-color-select'):readonlyColor(player.playerColor)}<span class="ready-badge ${player.isReady?'is-ready':'is-not-ready'}">${player.isReady?'READY':'NOT READY'}</span></span></li>`;
