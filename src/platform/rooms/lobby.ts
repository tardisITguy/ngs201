...
export interface Lobby{room:{id:string;code:string;status:string;gameId:string;maxPlayers:number;joinMode:'public'|'code';roomName:string|null;isCurrentUserHost:boolean};players:LobbyPlayer[]}
...
export async function getLobby(roomCode:string,client=getSupabaseClient()):Promise<Lobby>{
 ...
 const roomResult=await client.from('rooms').select('id,code,status,game_id,host_user_id,join_mode,room_name,games(max_players)').eq('code',code).maybeSingle();if(roomResult.error||!roomResult.data)throw new LobbyReadError();const room=roomResult.data as Record<string,unknown>,game=Array.isArray(room.games)?room.games[0]:room.games,validRoomName=room.room_name===null||(typeof room.room_name==='string'&&room.room_name===room.room_name.trim()&&[...room.room_name].length>=1&&[...room.room_name].length<=50&&!/[\u0000-\u001f\u007f]/.test(room.room_name));if(typeof room.id!=='string'||typeof room.code!=='string'||typeof room.status!=='string'||typeof room.game_id!=='string'||typeof room.host_user_id!=='string'||(room.join_mode!=='public'&&room.join_mode!=='code')||!validRoomName||!game||typeof game!=='object'||!Number.isInteger((game as Record<string,unknown>).max_players))throw new LobbyReadError();
 ...
 return{room:{id:room.id,code:room.code,status:room.status,gameId:room.game_id,maxPlayers:(game as Record<string,unknown>).max_players as number,joinMode:room.join_mode,roomName:room.room_name as string|null,isCurrentUserHost:room.host_user_id===user.id},players:[...humans,...bots]};
}
