...
export interface Lobby{room:{id:string;code:string;status:string;gameId:string;maxPlayers:number;isCurrentUserHost:boolean};players:LobbyPlayer[]}
...
export async function getLobby(roomCode:string,client=getSupabaseClient()):Promise<Lobby>{
 ...
 return{room:{id:room.id,code:room.code,status:room.status,gameId:room.game_id,maxPlayers:(game as Record<string,unknown>).max_players as number,isCurrentUserHost:room.host_user_id===user.id},players:[...humans,...bots]};
}
