...
Deno.serve(async request=>{
 ...
 if(!payload||typeof payload!=='object'||Array.isArray(payload))return response(400,{error:'Invalid color request'});
 const record=payload as Record<string,unknown>,keys=Object.keys(record);
 if(keys.some(key=>key!=='roomCode'&&key!=='playerColor')||!keys.includes('roomCode')||!keys.includes('playerColor'))return response(400,{error:'Invalid color request'});
 const {roomCode,playerColor}=record;
 if(typeof roomCode!=='string')return response(400,{error:'Invalid color request'});
 const normalizedRoomCode=roomCode.trim().toUpperCase();
 if(!/^[A-Z0-9]{6,10}$/.test(normalizedRoomCode))return response(400,{error:'Invalid color request'});
 if(playerColor!==null&&(typeof playerColor!=='string'||playerColor.trim()!==playerColor||!/^[a-z][a-z0-9-]{0,39}$/.test(playerColor)))return response(400,{error:'Invalid color request'});
 ...
 const {data,error}=await serverClient.rpc('set_player_color_server',{p_room_code:normalizedRoomCode,p_user_id:user.id,p_player_color:playerColor});
 if(error){
  if(error.code==='22023')return response(400,{error:'Invalid color request'});
  if(error.code==='P0002')return response(404,{error:'Room not found or unavailable'});
  if(error.code==='P0003')return response(409,{error:'That color is no longer available.'});
  return response(500,{error:'Unable to update player color'});
 }
 const result=Array.isArray(data)?data[0]:data;
 if(!result||typeof result!=='object')return response(500,{error:'Unable to update player color'});
 const value=result as Record<string,unknown>;
 if(typeof value.room_code!=='string'||(value.player_color!==null&&typeof value.player_color!=='string')||typeof value.changed!=='boolean')return response(500,{error:'Unable to update player color'});
 return response(200,{roomCode:value.room_code,playerColor:value.player_color,changed:value.changed});
});
