...
Deno.serve(async request=>{
 ...
 if(!Array.isArray(data))return response(500,{error:'Unable to load rooms'});const rooms=data.map(value=>{const row=value as Record<string,unknown>;return{roomCode:row.room_code,hostDisplayName:row.host_display_name,humanPlayers:row.human_players,aiPlayers:row.ai_players,totalPlayers:row.total_players,maxPlayers:row.max_players,createdAt:row.created_at};});return response(200,{rooms});
});
